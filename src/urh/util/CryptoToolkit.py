...
def porsche_cayenne_decrypt(pkt_64bit):
    ...
    def _encrypt(serial24, btn_byte, cnt_lo, cnt_hi):
        ...
        a9A = r_h ^ b0
        p1 = ((~cnt_lo << 2) & 0xFC) ^ r_m; p1 &= 0xCC
        p2 = ((~cnt_hi << 2) & 0xFC) ^ r_m; p2 &= 0x30
        p3 = ((~cnt_hi >> 6) & 0x03) ^ r_m; p3 &= 0x03
        a9B = (p1 | p2 | p3) & 0xFF
        q1 = ((~cnt_lo >> 2) & 0x3F) ^ r_l; q1 &= 0x33
        q2 = (((~cnt_hi) & 0x03) << 6) ^ r_l; q2 &= 0xC0
        q3 = ((~cnt_hi >> 2) & 0x3F) ^ r_l; q3 &= 0x0C
        a9C = (q1 | q2 | q3) & 0xFF
        o4 = ((a9A >> 2) & 0x3F) | (((~cnt_lo) & 0x03) << 6)
        o5 = ((~cnt_lo) & 0xC0) | ((a9A & 0x03) << 4) | (a9B & 0x0C) | (((~cnt_lo) >> 2) & 0x03)
...
_SECV2_ORDER = {
    0x02: (0, 1, 2), 0x0A: (0, 1, 2),
    0x00: (0, 2, 1), 0x05: (1, 0, 2),
    0x06: (2, 1, 0), 0x09: (2, 1, 0),
    0x08: (1, 2, 0), 0x04: (1, 2, 0),
    0x01: (2, 0, 1),
}
...
ALUTECH_TABLE = [
    ...
    0x22, 0x07, 0x68, 0x4D, 0xB6, 0x93, 0xFC, 0xD9,
    0x44, 0x61, 0x0E, 0x2B, 0xD0, 0xF5, 0x9A, 0xBF,
...
def kia_v3_v4_derive_key(serial):
    ...
    hop_high = keeloq_encrypt(
        msb16 | 0x60000000, KIA_V3_V4_MASTER_KEY
    )
...
KIA_V6_AES_KEY = [
    0x00, 0x01, 0x02, 0x13, 0x04, 0x05, 0x06, 0x07,
...
def faac_slh_derive_key(serial, seed, manufacturer_key):
    ...
    hop_high = keeloq_encrypt(
        (seed >> 16) | (serial << 16), manufacturer_key
    )
    return (hop_high << 32) | hop_low
...
VAG_TYPE_NAMES = {
    0x00: "VAG NEW", 0xC0: "VAG OLD", 0xC1: "AUDI",
    0xC2: "SEAT", 0xC3: "SKODA",
}
...
def vag_decode(key1_64bit, key2_16bit):
    ...
    if btn_nib in (1, 2, 4, 8) and dispatch in VAG_DISPATCH_T12:
        btn = btn_nib << 4
        serial_raw = tea_dec[0] | (tea_dec[1] << 8) | (tea_dec[2] << 16) | (tea_dec[3] << 24)
        serial = (
            ((serial_raw >> 24) & 0xFF)
            | (((serial_raw >> 16) & 0xFF) << 8)
            | (((serial_raw >> 8) & 0xFF) << 16)
...
def vag_decrypt(
    encrypted_v0, encrypted_v1, key, vag_type=2,
    sbox=None, pbox=None,
):
    """
    ...
    if vag_type in (1, 3):
        ...
        tea_key = list(key) if len(key) == 4 else [
            (key[i * 4] << 24)
            | (key[i * 4 + 1] << 16)
            | (key[i * 4 + 2] << 8)
            | key[i * 4 + 3]
